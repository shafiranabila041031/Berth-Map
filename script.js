...
            const finalLeft = left; // <--- INI PERBAIKAN BUG #1
...
        grid.innerHTML = ''; 
        const gridContainer = grid.parentElement; // <-- PERBAIKAN BUG #2
...
        const totalGridWidth = totalHours * HOUR_WIDTH; // <-- PERBAIKAN BUG #2
...
        grid.style.width = `${totalGridWidth}px`; // <-- PERBAIKAN BUG #2
...
        });
        ...
        /* --- GANTI SELURUH BLOK DARI 'const berths' SAMPAI AKHIR LOOP 'berths.forEach': --- */

        // 1. UBAH BATAS KD DI SINI SESUAI PERMINTAAN ANDA
        const berths = [
...
            innerLabelWrapper.className = 'berth-label'; // Class ini sudah punya style rotate()

            // Kalkulasi posisi dan tinggi (ini dipindah ke atas)
...
            // 2. UBAH STYLE WRAPPER (PAKAI 'space-evenly' UNTUK "SAMA RATA")
            innerLabelWrapper.style.display = 'flex';
            innerLabelWrapper.style.flexDirection = 'row'; 
            innerLabelWrapper.style.width = `${height}px`; 
            innerLabelWrapper.style.justifyContent = 'space-evenly'; // <-- Ini artinya "sama rata"
...
            // 3. UBAH URUTAN ELEMEN (SESUAI GAMBAR ANDA)
        
            // Elemen Pertama (misal: CC04 / CC02)
            const ccEl_Top = document.createElement('div');
            ccEl_Top.textContent = berth.ccs[1]; // <- Pakai ccs[1]
            ccEl_Top.style.fontSize = '0.9em';
            ccEl_Top.style.color = 'red';
            innerLabelWrapper.appendChild(ccEl_Top);
        
            // Elemen Tengah (Nama Berth)
...
            // Elemen Terakhir (misal: CC03 / CC01)
            const ccEl_Bottom = document.createElement('div');
            ccEl_Bottom.textContent = berth.ccs[0]; // <- Pakai ccs[0]
            ccEl_Bottom.style.fontSize = '0.9em';
            ccEl_Bottom.style.color = 'red';
...
        });
/* --- BATAS AKHIR KODE PERBAIKAN --- */

        // SECARA PAKSA MEMPERBAIKI CSS YANG SALAH (width: 100%)
        gridContainer.style.width = `${totalGridWidth}px`; // <-- PERBAIKAN BUG #2
        xAxis.style.width = `${totalGridWidth}px`; // <-- PERBAIKAN BUG #2
